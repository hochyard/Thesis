...
def build_DNN_model(number_features, LR):
    ...
    model.add(layer=layers.Dense(units=number_features*0.2, activation='relu', input_shape=[number_features]))
    model.add(layers.Dropout(0.1))
    model.add(layer=layers.Dense(units=number_features*0.1, activation='relu'))
    model.add(layers.Dropout(0.1))
...
def train_DNN_incremental(x_train_chunks_file, y_train_chunks_file, X_val, y_val, batch_size, num_epochs, LR, directory_to_save):
    ...
    title = 'Autoencoder + DNN Loss on Height'
    plot_loss(loss, val_loss, title, save_name)
    return model

def train_XGBRegressor_incremental(x_train_chunks_file, y_train_chunks_file, X_val, y_val, max_depth, num_epochs, LR, colsample_bytree, gamma, directory_to_save):
    """
    Train XGBRegressor. The model is learning incrementally, for each epoch, the model build by each chunk, one gradient boosted tree.
    Note: the samples of x and y should be arranged in the same order according to the ID.
    :param x_train_chunks_file (str): file name with the train data saved in chunks.
    :param y_train_chunks_file (str): file name with the train target data saved in chunks.
    :param X_val (df):  validation set
    ...
    :return: trained XGBRegressor model.
    """
    loss = []
    val_loss = []
    model = xgboost.XGBRegressor(n_estimators=1, max_depth=max_depth, learning_rate=LR, colsample_bytree=colsample_bytree, gamma=gamma)
    ...
    for epoch in range(1,num_epochs+1):
                ...
                while True:
                    try:
                        ...
                        if batch_num == 2 and epoch==1:
                            model.fit(X_batch, y_batch, eval_set=[(X_batch, y_batch), (X_val, y_val)], verbose=True)
                        else:
                            model.fit(X_batch, y_batch, xgb_model=model, eval_set=[(X_batch, y_batch), (X_val, y_val)], verbose=True)
                        batch_num = batch_num + 1
        ...
        # check early stopping - after 6
        if epoch == 1:
            best_result = results['validation_1']['rmse']
            best_result_counter = 0
            continue
        if best_result > results['validation_1']['rmse']:
    ...
    title = 'Autoencoder + XGB Loss on Height'
    plot_loss(loss, val_loss, title, save_name)
    return model
...
if __name__ == '__main__':
    X_train_chunks_dim_remove_file = "/home/hochyard/my_model/autoencoder/autoencoder_models_5_layers_prelu_act_no_cov_adam0.00001_batch_size250/X_train_1k_chunks_dim_remove_no_missing_600_epochs/height_all_chr_MinMax_scaled_cov_MinMax_scaled_no_40PCA.pkl" #for Autoencoder
    X_test_chunks_dim_remove_file = "/home/hochyard/my_model/autoencoder/autoencoder_models_5_layers_prelu_act_no_cov_adam0.00001_batch_size250/X_test_1k_chunks_dim_remove_no_missing_600_epochs/height_all_chr_MinMax_scaled_cov_MinMax_scaled_no_40PCA.pkl" #for Autoencoder
    ...
    DNN_params = {'batch_size': 50,
                  'num_epochs': 80,
                  'LR': 0.0000001,
                  'directory_to_save': "/home/hochyard/my_model/autoencoder/autoencoder_models_5_layers_prelu_act_no_cov_adam0.00001_batch_size250/height_pheno/NN/NN_0.2_dropout_0.1_dropout_relu_Adam0.0000001_batch_size_50_genes_MinMax_scaled_cov_MinMax_scaled_no_40_PCA/"}
    ...
    XGB_params = {'max_depth': 5,
                  ...
                  'gamma': 0.01,
                  'directory_to_save': "/home/hochyard/my_model/autoencoder/autoencoder_models_5_layers_prelu_act_no_cov_adam0.00001_batch_size250/height_pheno/XGB/incremental_xgboost_regressor_MinMax_scaled_cov_MinMax_scaled_max_depth5_learning_rate0.001_subsample1.0_olsample_bytree0.2_gamma0.01_no_40_PCA/"}

    XGB_model = train_XGBRegressor_incremental(x_train_chunks_file=X_train_chunks_dim_remove_file,
                                               y_train_chunks_file=y_train_chunks_file,
                                               X_val=X_val,
                                               y_val=y_val,
                                               **XGB_params)
